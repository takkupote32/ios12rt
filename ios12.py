...
# ========================================================
# 👇 【iOS 12.5.8 Safari絶対防御】古いWebkit用の超単純化CSS
# ========================================================
st.markdown("""
    <style>
    /* 右下の「Manage app」ボタンを非表示 */
...
# ========================================================
# セッション状態（データ永続化・記憶用変数）の初期化 [cite: 2026-01-20]
# ========================================================
...
with col1:
    ...
    # 負担割合 (iOS12対策: segmented_control から radio へ変更)
    ratio_str = st.radio("患者負担割合", ["1割", "2割", "3割"], index=2, horizontal=True)
    ...
    # 固定器具加算フレーム (iOS12対策: segmented_control から radio へ変更)
    grid_fix_list = ["前立腺VMAT", "IMRT", "1門照射", "対向2門", "非対向2門・3門照射", "4門以上・運動・原体照射", "ケロイド"]
    default_fix_idx = 1 if selected_method == "前立腺VMAT" else 0
    fix_var = "なし"
    if selected_method in grid_fix_list:
        fix_var = st.radio("体外照射用固定器具", ["なし", "作成あり"], index=default_fix_idx, horizontal=True)

    # 呼吸性移動対策フレーム (iOS12対策: segmented_control から radio へ変更)
    breath_var = "なし"
    if selected_method in ["IMRT", "肺定位SBRT", "全乳房(一連)", "非対向2門・3門照射", "4門以上・運動・原体照射"]:
        breath_var = st.radio("呼吸性移動対策", ["なし", "あり"], index=0, horizontal=True)

    # 全乳房専用IGRTフレーム (iOS12対策: segmented_control から radio へ変更)
    breast_igrt_var = "なし"
    if selected_method == "全乳房(一連)":
        breast_igrt_var = st.radio("IGRT(一連2400点)", ["なし", "あり"], index=0, horizontal=True)
...
with col2:
    ...
    # iOS 12対策: st.htmlのCSS崩れを完全に防ぐため、標準の st.code (等幅フォント・スクロール自動) に変更
    st.code(full_formula_text, language="text")
...
if is_expanded:
    ...
    with ex_col1:
        ...
        if check_extra_2:
            ...
            if selected_extra_method in ["非対向2門・3門照射", "4門以上・運動・原体照射"]:
                # iOS12対策: segmented_control -> radio
                default_ex_breath_idx = 1 if st.session_state.saved_extra_2nd_breath == "あり" else 0
                extra_breath_var = st.radio("追加 呼吸性移動対策", ["なし", "あり"], index=default_ex_breath_idx, horizontal=True, key="ex_breath_2")
            ...
            if selected_extra_method in ["IMRT", "非対向2門・3門照射", "4門以上・運動・原体照射"]:
                # iOS12対策: segmented_control -> radio
                default_ex_breath_idx = 1 if st.session_state.saved_extra_1st_breath == "あり" else 0
                extra_breath_var = st.radio("追加 呼吸性移動対策", ["なし", "あり"], index=default_ex_breath_idx, horizontal=True, key="ex_breath_1")
    ...
    def get_extra_points(method, count, igrt_selection, is_second):
        ...
        if method in one_series_methods:
            ...
            prefix = "2回目" if is_second else "追加"
            formula_list_ex.append(f"--- 照射ごとの計算 ---")
    ...
    with ex_col2:
        ...
        # iOS 12対策: st.html から st.code に変更
        st.code(ex_full_txt, language="text")
